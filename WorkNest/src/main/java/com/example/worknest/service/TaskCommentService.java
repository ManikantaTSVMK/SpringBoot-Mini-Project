...
public class TaskCommentService {
    ...
    public TaskComment add(Long taskId, Long authorUserId, String content) {
        ...
        User author = userRepo.findById(authorUserId)
        ...
        // ✅ Use helper method to maintain both sides of the relationship
        task.addComment(comment);

        // ✅ Save only the Task, cascade will save the comment
        taskRepo.save(task);
    ...
    public List<TaskComment> listByTask(Long taskId) {
        Task task = taskRepo.findById(taskId)
                .orElseThrow(() -> new IllegalArgumentException("Task not found"));
        return task.getComments(); // ✅ returns managed collection
    }
}
